...
const EditLenda = () => {
  ...
  useEffect(() => {
    setCtxIds(getAdminIdsFromStorage());
  }, []);

  // 2) Fetch: universiteti, departamenti, semestri + lenda
  useEffect(() => {
    ...
    const fetchLenda = async () => {
      try {
        ...
        setLendaData({
          Emri: data.emri ?? data.Emri ?? "",
          Kredite: String(data.kredite ?? data.Kredite ?? ""),
          // Semestri do të shfaqet si ID (UI mund të ndryshohet)
          SemestriId: String(data.semestriId ?? data.SemestriId ?? ""),
          Kategoria: data.kategoria ?? data.Kategoria ?? ""
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      DepartamentiId: toIntOrNull(ctxIds.dep),  // nga LS – nuk lejohet ta ndryshojë UI
      SemestriId: toIntOrNull(lendaData.SemestriId),
      Kategoria: lendaData.Kategoria
    };

    try {
      await axiosClient.put(`/lenda/${id}`, payload);
      alert('Lënda u përditësua me sukses!');
      navigate('/lenda');
    } catch (error) {
      const msg =
        ...
        error.message;
      console.error('Gabim:', error?.response || error);
      alert('Gabim gjatë përditësimit të lëndës:\n' + msg);
    }
  };
        ...
        <div id="layoutSidenav_content">
          <main>
            <div className="container mt-5">
              <h2 className="mb-4">Përditëso Lëndën</h2>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn btn-primary mt-3">Përditëso</button>
              </form>
            </div>
          </main>
        </div>
