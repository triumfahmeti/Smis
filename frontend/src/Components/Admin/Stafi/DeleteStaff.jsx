...
const DeleteStaf = () => {
  const [stafi, setStafi] = useState(null);
  const navigate = useNavigate();
  const { id } = useParams();

  // Merr stafin sipas ID
  useEffect(() => {
    async function fetchStafi() {
      try {
        const { data } = await axiosClient.get(`/stafiakademik/${id}`);
        setStafi(data);
      } catch (error) {
        console.error("Gabim gjatë marrjes së stafit:", error.response?.data || error.message);
        alert("Stafi nuk u gjet ose ka problem me autorizimin.");
  ...
  const handleDelete = async () => {
    if (!window.confirm("A jeni i sigurt që doni ta fshini këtë staf?")) return;

    try {
      await axiosClient.delete(`/stafiakademik/${id}`);
      alert("Stafi dhe user-i u fshinë me sukses.");
      navigate('/stafiakademik');
    } catch (error) {
      console.error("Gabim gjatë fshirjes:", error.response?.data || error.message);
      alert("Gabim gjatë fshirjes së stafit. Kontrollo autorizimin ose endpointin.");
    }
  };
        ...
        <div id="layoutSidenav_content">
          <main>
            <div className="container mt-5">
              <h2 className="mb-4">Fshi Stafin</h2>
              {stafi ? (
                <div>
                  <p>
                    A jeni i sigurt që doni të fshini stafin{" "}
                    <strong>{stafi.emri} {stafi.mbiemri}</strong>?
                  </p>
                  <button onClick={handleDelete} className="btn btn-danger">Po, fshije</button>
                  <Link to="/stafiakademik" className="btn btn-secondary ms-2">Anulo</Link>
                </div>
              ) : (
                <p>Po ngarkohet stafi...</p>
              )}
            </div>
          </main>
        </div>
